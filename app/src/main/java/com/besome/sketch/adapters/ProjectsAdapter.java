...
public class ProjectsAdapter extends RecyclerView.Adapter<ProjectsAdapter.ProjectViewHolder> {
    ...
    private void showProjectOptionsBottomSheet(HashMap<String, Object> projectMap, int position) {
        ...
        binding.projectCleanup.setOnClickListener(v -> {
            DayDreamCleanUpTemporaryFiles.showDialogNow(activity, yB.c(projectMap, "sc_id"));
            projectOptionsBSD.dismiss();
        });
        ...
        if (LibraryUtils.isAllowUseGit()) {
            binding.projectGit.setOnClickListener(v -> {
                Intent intent = new Intent(activity, DayDreamGitActionsActivity.class);
                intent.putExtra("sc_id", yB.c(projectMap, "sc_id"));
                activity.startActivity(intent);
                projectOptionsBSD.dismiss();
            });
