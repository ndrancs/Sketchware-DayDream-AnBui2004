...
import extensions.anbui.daydream.theme.DayDreamThemeTool;
...
public class yq {
    ...
    public void generateDebugFiles(Context context) {
        ...
        if (!targetApplicationFile.exists()) {
            ...
            if (DRFeatureManager.isDynamicColorEnabled(sc_id) || new Material3LibraryManager(sc_id).isDynamicColorsEnabled()) {
                sketchApplicationFileContent = sketchApplicationFileContent.replace(
    ...
    public void a(iC projectLibraryManager, hC projectFileManager, eC projectDataManager, ExportType exportingType) {
        ...
        if (DRFeatureManager.isNeedAddNetworkPermision(sc_id)) {
            N.addPermission(jq.PERMISSION_INTERNET);
            N.addPermission(jq.PERMISSION_ACCESS_NETWORK_STATE);
        }
    ...
    public String getXMLStyle() {
        ...
        if (DRFeatureManager.isThemeEnabled(sc_id)) {
            XmlBuilderHelper stylesFileBuilder = new XmlBuilderHelper();
            if (DayDreamProjectSettings.getTheme(sc_id) > Configs.material2Theme) {
                stylesFileBuilder.addStyle("AppTheme", DayDreamThemeTool.generateCodeForMaterialType(sc_id) + ".NoActionBar");
                ...
                stylesFileBuilder.addStyle("AppTheme", DayDreamThemeTool.generateCodeForMaterialType(sc_id) + ".NoActionBar" + (useNewMaterialComponentsTheme ? "" : ".Bridge"));
                ...
                //Theme.Material != Theme.MaterialCompoment.
                //Theme.Material = native theme in Android framework, no need for AndroidX.
